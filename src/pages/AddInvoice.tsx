...
export default function AddInvoice() {
  ...
  return <form onSubmit={submit}>
    ...
    <section className="invoice-form-card">
      ...
      <h2>Client Detail</h2>
      <div className="formgrid three">
        {['clientName', 'companyName', 'phoneNumber', 'email'].map((field) => <label key={field}>{field === 'clientName' ? 'Client Name' : field === 'companyName' ? 'Company Name' : field === 'phoneNumber' ? 'Phone Number' : 'Email'}<input required={field === 'clientName'} value={values[field] || ''} onChange={(e) => set(field, e.target.value)} placeholder={`Enter ${field.replace(/([A-Z])/g, ' $1')}`} /></label>)}
      ...
      <div className="tablewrap invoice-items"><table><thead><tr>{['Sr no.', 'Item Code', 'Product', 'QTY', 'Unit Price', 'Discount', 'Total', 'Actions'].map((h) => <th key={h}>{h}</th>)}</tr></thead><tbody>
        {items.map((item, index) => <tr key={index}><td>{index + 1}</td><td><input value={item.code} onChange={(e) => updateItem(index, 'code', e.target.value)} placeholder="33" /></td><td><select value={item.product} onChange={(e) => updateItem(index, 'product', e.target.value)}><option value="">Select Product</option><option>Growth Max 30kg</option><option>Maintaince Pro</option></select></td><td><input type="number" min="1" value={item.qty} onChange={(e) => updateItem(index, 'qty', e.target.value)} /></td><td><input type="number" min="0" value={item.price} onChange={(e) => updateItem(index, 'price', e.target.value)} placeholder="4500" /></td><td><input type="number" min="0" value={item.discount} onChange={(e) => updateItem(index, 'discount', e.target.value)} placeholder="100" /></td><td>{itemTotal(item).toLocaleString()}</td><td><button type="button" className="line-delete" onClick={() => setItems((current) => current.length === 1 ? current : current.filter((_, i) => i !== index))}><Trash2 /></button></td></tr>)}
