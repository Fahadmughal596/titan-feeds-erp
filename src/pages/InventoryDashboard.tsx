...
export default function InventoryDashboard() {
  ...
  return <><PageHeader title="INVENTORY" /><div className="tabs inventory-tabs"><button className="tab active">Inventory</button><button className="tab" onClick={() => setTab('batches')}>Batches</button></div><div className="inventory-stats"><div><h3>Raw Material</h3><strong>120M</strong><span>Total Stock Value</span><strong>192Ton</strong><span>Total Stock Weight</span></div><div><h3>Finish Products</h3><strong>400</strong><span>Bags</span><strong>100Ton</strong><span>In Kg</span></div><div><h3>Low Stock</h3><strong>7</strong><span>Raw Material</span><strong>1</strong><span>Finish Goods</span></div></div><div className="toolbar inventory-toolbar"><button className="btn green" onClick={() => exportCsv('titan-inventory.csv', ['Name', 'Category', 'Available QTY', 'Wight', 'Price', 'Status'], stockRows)}><Download />Export</button><button className="btn orange" onClick={() => setFilterOpen((open) => !open)}><Filter />Filter</button>{filterOpen && <><select value={category} onChange={(e) => setCategory(e.target.value)}><option value="">Select Category</option><option>Raw Material</option><option>Finish Goods</option></select><select value={status} onChange={(e) => setStatus(e.target.value)}><option value="">Select Status</option><option>Low Stock</option></select></>}</div><div className="tablewrap inventory-table"><table><thead><tr>{['Name', 'Category', 'Available QTY', 'Wight', 'Price', 'Status', 'Actions'].map((h) => <th key={h}>{h}</th>)}</tr></thead><tbody>{stock.map((r, i) => <tr key={i}><td>{r.name}</td><td>{r.category}</td><td>{r.qty}</td><td>{r.weight}</td><td>{r.price}</td><td><span className="inventory-low">{r.status}</span></td><td><button className="icon-button"><MoreVertical /></button></td></tr>)}</tbody></table></div><div className="products-pagination"><span>Page <select defaultValue="1"><option>1</option></select> of 10</span><div className="page-controls"><button>«</button><button>‹</button><button className="current">1</button><button>2</button><button>3</button><span>…</span><button>10</button><button>›</button><button>»</button></div></div></>;
}
