...
export default function PartnerForm({ kind }: { kind: 'client' | 'supplier' }) {
  ...
  const submit = (e: React.FormEvent) => { e.preventDefault(); localStorage.setItem(`last_${kind}`, JSON.stringify(values)); setSaved(true); window.setTimeout(() => navigate(ROUTES.CLIENTS_SUPPLIERS), 400); };
