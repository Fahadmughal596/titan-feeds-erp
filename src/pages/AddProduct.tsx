...
export default function AddProduct() {
  ...
  const submit = (e: React.FormEvent) => { e.preventDefault(); let rows: unknown = []; try { rows = JSON.parse(localStorage.getItem('titan_products_v4') || '[]'); } catch { /* start with empty catalogue */ } const list = Array.isArray(rows) && (rows.length === 0 || Array.isArray(rows[0])) ? rows as string[][] : []; list.push([brand, name, '-', '', 'Bag', '', description || '-']); localStorage.setItem('titan_products_v4', JSON.stringify(list)); navigate(ROUTES.PRODUCTS); };
